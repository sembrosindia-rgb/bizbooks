        ...
        if 'signup' in request.POST:
            ...
            if serializer.is_valid():
                ...
                if settings and (settings.email_verification_required or settings.phone_verification_required):
                    return redirect('/welcome/?message=Account created! Please verify your email/phone.')
                else:
                    from rest_framework.authtoken.models import Token
                    token, _ = Token.objects.get_or_create(user=user)
                    return redirect(f'/welcome/?token={token.key}')
            else:
            ...
            if serializer.is_valid():
                ...
                from rest_framework.authtoken.models import Token
                token, _ = Token.objects.get_or_create(user=user)
                return redirect(f'/dashboard/?token={token.key}')
            else:
...
def welcome_view(request):
    message = request.GET.get('message', 'Welcome! You can now use your account.')
    ...
    <body>
        <div class="container">
            <h1>🎉 Welcome to BizBooks!</h1>
            <p>{message}</p>
            {'<p>Your API Token: <code>' + token + '</code></p>' if token else ''}
...
def dashboard_view(request):
    ...
    <body>
        <div class="container">
            <h1>📊 Dashboard</h1>
            <p>Hello, {request.user.username}! You are logged in.</p>
            {'<p>Your API Token: <code>' + token + '</code></p>' if token else ''}
            <p><a href="/">Home</a></p>
        </div>
    </body>
